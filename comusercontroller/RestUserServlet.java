...
public class RestUserServlet extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		userRestDto.setUserReason(req.getParameter("userReason"));
		userRestDto.setUserId(Integer.parseInt(req.getParameter("userId")));
		
		//入力チェック
		ArrayList<String> errorMessageList = new ArrayList<String>();
		if ("".equals(userRestDto.getUserReason())) {
			errorMessageList.add("休止理由を入力してください");
		}
		if (errorMessageList.size() != 0) {
			//利用者一覧に遷移
			req.setAttribute("errorMessageList", errorMessageList);
			req.setAttribute("userReason", userRestDto.getUserReason());
		...
		try(Connection conn =DataSourceManager.getConnection()) {
			...
			userRestDao.statusOff(userRestDto);
